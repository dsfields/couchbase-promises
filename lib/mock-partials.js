...
const assertLeaf = function(leaf, path) {
  if (typeof leaf === 'undefined')
    throw new TypeError(`One or more nodes in the path "{path}" do not exists`);
};
...
class MutateInBuilder {
  ...
  addUnique(path, value, createParents) {
    return this.arrayAddUnique(path, value, createParents);
  }

  pushBack(path, value, createParents) {
    return this.arrayAppend(path, value, createParents);
  }

  pushFront(path, value, createParents) {
    return this.arrayPrepend(path, value, createParents);
  }
}
