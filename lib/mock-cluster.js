...
const lookupIn = function (key) {
  return new partials.LookupInBuilder(this, key);
};


const mutateIn = function (key) {
...
const listAppend = function (key, value, options, callback) {
  ...
  this.get(key, (err, res) => {
    if (isFailure(err)) {
      if (elv(options) && options.createList) {
        ...
        return self.insert(key, newList, (e, r) => {
          ...
          return cb(undefined, { cas: r.cas, value: newList });
        });
      }
      return cb(err);
    }
    ...
    return self.replace(key, val, (e, r) => {
      if (elv(e)) return cb(e);
      return cb(undefined, { cas: r.cas, value: val });
    });
  });
};
...
const listGet = function (key, index, options, callback) {
  ...
  this.get(key, (err, res) => {
    ...
    if (index >= res.value.length) {
      return cb(new couchbase.Error('Index out of range'));
    }

    return cb(undefined, { cas: res.cas, value: res.value[index] });
  });
};
